...
global.REQUEST = METHOD({
	...
	run : function(params, responseListenerOrListeners) {
		...
		var isSecure = params.isSecure === undefined ? BROWSER_CONFIG.isSecure : params.isSecure;
		var host = params.host === undefined ? BROWSER_CONFIG.host : params.host;
		var port = params.port === undefined ? (params.host === undefined ? BROWSER_CONFIG.port : 80) : params.port;
		var uri = params.uri;
		var url = params.url;
		var paramStr = params.paramStr;
...
if (window.AudioContext === undefined) {
	...
	global.SOUND = CLASS(function(cls) {
		...
		var audioContext;
		var isCanPlayOGG = new Audio().canPlayType('audio/ogg') !== '';
		...
		return {
			...
			init : function(inner, self, params) {
				...
				var src = isCanPlayOGG === true ? ogg : mp3;
				if (src === undefined) {
					src = wav;
				}
				...
				var ready = function() {
					...
					var binary = atob(src.substring(src.indexOf(',') + 1));
					...
					audioContext.decodeAudioData(bytes.buffer, function(_buffer) {
				...
				var setVolume = self.setVolume = function(_volume) {
					//REQUIRED: _volume
				...
				var setPlaybackRate = self.setPlaybackRate = function(playbackRate) {
